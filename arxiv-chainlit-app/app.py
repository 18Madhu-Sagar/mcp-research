...
class ArxivMCPClient:
    ...
    async def search_papers(self, query: str, max_results: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # Use session with proper headers and context management
            with requests.Session() as session:
                session.headers.update({
                    'Accept-Encoding': 'identity',  # Disable compression
                    'User-Agent': 'ArxivChainlitApp/1.0'
                })
                
                with closing(session.get(self.base_url, params=params, timeout=30)) as response:
...
"""
            ...
            # Use chat completions API directly (more reliable)
            try:
                ...
                analysis_content = f"Error calling Mistral API: {str(api_error)}"
...
"""
            ...
            try:
                ...
                print(f"Mistral API error: {api_error}")
                response_content = f"Error calling Mistral API: {str(api_error)}"
    ...
    async def _fetch_paper_details(self, arxiv_id: str) -> Dict:
        ...
        try:
            ...
            with requests.Session() as session:
                ...
                with closing(session.get(url, timeout=30)) as response:
                    response.raise_for_status()
                    content = response.content
            ...
            print(f"Network error fetching paper details: {e}")
            return None
        except ET.ParseError as e:
            print(f"XML parsing error for paper details: {e}")
            return None
        except Exception as e:
            print(f"Unexpected error fetching paper details: {e}")
            return None
